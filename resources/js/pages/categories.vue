...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Todas as Categorias"> 
        <v-btn class="justify-content: left"> Criar Categoria</v-btn>
        <Table
            :dados="categories.data"
...
export default {
    data() {
        return {
            urlBase: 'http://localhost:8000/api/categories',
            name: "",
            description: "",
            slug: "",
            image: "",
    ...
    methods: {
        search() {
            ...
            axios.get('http://localhost:9011/api/v1/customer/'+this.search.id)
                .then(response => {
                    this.customers = response.data.item
